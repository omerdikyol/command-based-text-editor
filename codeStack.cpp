...
void codeStack::pushToStack(const char* s) {
    stackNode *node = new stackNode(s);         // We created our new node.
    node -> next = NULL;
    node -> next = top;     // Put it in the linked list Stack
    top = node;             // Made it last element
    return;
}
...
void codeStack::printStack() {
    codeStack tempStack;
     while(!isEmpty())
    {
        string s = popFromStack();
        const char* c = s.c_str();
        tempStack.pushToStack(c);
        cout << s;
    }
    while(!tempStack.isEmpty())
    {
        string s = tempStack.popFromStack();
        const char* c = s.c_str();
        pushToStack(c);
    }
    return;
